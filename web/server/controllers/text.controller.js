...
exports.getText = async (req, res) => {
  ...
  if (lang.length !== 2) return res.status(500).send("Received incorrect data");

  if (!fs.existsSync(process.env.DATA_PATH + process.env.TEXTS_FILENAME))
    return res.status(500).send("Internal error");

  const texts = JSON.parse(
    fs.readFileSync(process.env.DATA_PATH + process.env.TEXTS_FILENAME)
  );

  const index = texts.findIndex((text) => text.lang === lang);

  if (index === -1) return res.status(500).send("Internal error");
